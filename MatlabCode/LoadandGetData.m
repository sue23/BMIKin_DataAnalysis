...
cursor=repmat({NaN},length(soggetti),10,ng);
start=repmat({NaN},length(soggetti),10,ng);
stop=repmat({NaN},length(soggetti),10,ng);
Init=repmat({NaN},length(soggetti),10,ng);
Stop=repmat({NaN},length(soggetti),10,ng);
...
%%        
        ...
        for kk = 1:nepochs % per block in each session
            ...
            [out_findmovement]=findmovement(stopIndx_o(kk),startIndx_o(kk),Datao,A_posture,subj,sess,soggetti,g,kk,KinOffset.offsetCalibDataMap);
            if length(out_findmovement.reachcompletedata)>1%~isnan(out_findmovement.reachcompletedata)
                [ cursor{subj,sess,g} ] = ReconstructFromBody(out_findmovement.reachkinectdataK,A,Rotation,Gain,Offset);
                [ Null_cursor{subj,sess,g} ] = ReconstructNullSpace(out_findmovement.reachkinectdataK,A,Rotation,Gain,Offset);
                ...
                start{subj,sess,g}=out_findmovement.start;
                stop{subj,sess,g}=out_findmovement.stop;
                Init{subj,sess,g}=out_findmovement.Init;
                Stop{subj,sess,g}=out_findmovement.Stop;
                % this is to compute reach metrics also for the
                % reconstructed trajectories
                
                rec.reachcompletedata(:,1) = out_findmovement.reachcompletedata(:,1);
                rec.reachcompletedata(:,2:3) = cursor{subj,sess,g}';
                rec.start = out_findmovement.start;
                rec.stop = out_findmovement.stop;
                [rec_computereachmetrics]=computereachmetrics(rec,g);
...
%%        % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
        ...
        for kk = 1:nepochs % per block in each session
            ...
            [out_findmovement]=findmovement(stopIndx_v(kk),startIndx_v(kk),Datav,A_posture,subj,sess,soggetti,g,kk,KinOffset.offsetCalibDataMap);
 ...
 %%       % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
        ...
        if ~isempty(startIndx_c)
            ...
            for kk = 1:nepochs % per block in each session
                ...
                [out_findmovement]=findmovement(stopIndx_c(kk),startIndx_c(kk),Datac,A_posture,subj,sess,soggetti,g,kk,KinOffset.offsetCalibDataMap);
...
save_data({'ReachCompleteData','ReachKinectData','cursor','Null_cursor','start','stop','Init','Stop','Reach','ReachMetrics','Rec','RecMetrics','varcur',' Tvarcur','Nvarcur','varshouldL','varshouldR','varpelvisL','varpelvisR','Mappe'},ReachCompleteData,ReachKinectData,cursor,Null_cursor,start,stop,Init,Stop,Reach,ReachMetrics,Rec,RecMetrics,varcur, Tvarcur,Nvarcur,varshouldL,varshouldR,varpelvisL,varpelvisR,Mappe);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
