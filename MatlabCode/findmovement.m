function [out]=findmovement(stopIndx,startIndx,Data,A,subj,sess,soggetti,g,kk,offset)
%findmovement
%   OUTPUT:
...
% badind=find(diff(ReachData(:,1))>(2/30));
% if ~isempty(badind)
%     keyboard
% end
...
temp2 = find(ReachData(:,4)==2);%in center target
...
%         StopIndx(3)= size(reachcompletedata,1);
...
% %     h2.Color='b';
% %     end
%     if length(StopIndx)~=20 | length(InitIndx)~=20
%         keyboard
%     end
...
for trial=1:length(InitIndx)
    ...
    if (bad_trial( subj,sess,g,kk,trial ))
        reachcompletedata(InitIndx(trial):StopIndx(trial),:)=reachcompletedata(InitIndx(trial):StopIndx(trial),:).*nan;
        reachkinectdata(InitIndx(trial):StopIndx(trial),:)=reachkinectdata(InitIndx(trial):StopIndx(trial),:).*nan;
        try
...
% % keyboard
% % close all
% [xyzB]=TKinectInBody(A,kinectdata,start,stop);
% reachkinectdata = reshape(xyzB,size(kinectdata,1),size(kinectdata,2));
out.rtime=[mt.rtime];
% out.rtime(outlierindx)=nan;
% for i=1:length(outlierindx)
%     reachcompletedata(start(outlierindx(i)):stop(outlierindx(i)),:)=nan;
%     reachkinectdata(start(outlierindx(i)):stop(outlierindx(i)),:)=nan;
% end
out.reachcompletedata=reachcompletedata;
out.reachkinectdata=reachkinectdata;
out.reachkinectdataK=kinectdata;
out.start=start;
out.stop=stop;
out.Init=InitIndx;
out.Stop=StopIndx;
% out.outlierindx=outlierindx;
out.dur=[mt.dur];
